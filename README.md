...
| 🛰️ 巡检监控 | 自动检测群消息，支持关键词/正则/消息类型匹配，违规自动处理（禁言/踢出） |
...
| `/bot monitor <关键词> [@某人]` | `/bot 监控 <关键词> [@某人]` | 添加指令监控规则 | 管理员/群主/白名单用户 |
| `/bot unmonitor <规则ID\|关键词> [@某人]` | `/bot 取消监控 <规则ID\|关键词> [@某人]` | 删除指令监控规则 | 管理员/群主/白名单用户 |
...
| 配置项 | 类型 | 默认值 | 说明 |
|--------|------|--------|------|
| `sentinel_command_user_whitelist` | 列表 | [] | 指令规则白名单用户，允许这些用户使用监控指令 |
| `sentinel_command_mute_duration` | 文本 | 60 | 指令规则触发时的禁言时长（秒） |
| `sentinel_command_reply_message` | 列表 | [] | 指令规则触发时的回复消息列表 |
| `sentinel_command_kick_message` | 列表 | [] | 指令规则累计违规踢出时的提示消息 |
